...
A logistic-regression-style model scores 9 engineered features (from 20 raw
data points) to predict whether a given afternoon hour will be a good
wingfoil session, sends Telegram alerts, verifies its own predictions against
a real MeteoSwiss station, and retrains its weights automatically via
...
1. **`features.py`** — pulls 20 raw values from Open-Meteo (forecast or
   ...
   pressure, and engineers them into 16 named features (`engineer_features`).
   Two fetch paths: `fetch_raw` (live, ~3 day forecast) and
   `fetch_raw_historical` (date-range archive, used only by `backtest.py`) —
   they must stay feature-compatible since the same `engineer_features`
...
4. **`forecast_and_log.py`** (scheduled 07:00 & 10:00 CEST) — runs
   `features.py` + `model.py` over the live forecast, tiers each afternoon
   hour into GOOD/MARGINAL/UNLIKELY via thresholds in `weights.json`, sends a
   Telegram message, appends every scored hour to `logs/predictions.jsonl`
   with `verified: false`.
...
- `weights.json` is treated as a live, auto-updated artifact. Only hand-edit
  ...
  afterward will keep learning from wherever you leave it.
